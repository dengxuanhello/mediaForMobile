...
import org.m4m.effects.RecordVoiceEffect;
...
public class CameraCapturerActivity extends ActivityWithTimeline implements CameraCaptureSettingsPopup.CameraCaptureSettings {
    ...
    private TimelineItem myBackgroundMusicTimelineItem;
    ...
    private RecordVoiceEffect recordVoiceEffect;
    ...
    public void onCreate(Bundle icicle) {
        ...
        settingsPopup = new CameraCaptureSettingsPopup(this, supportedResolutions, this,previewWidth);
        ...
        myBackgroundMusicTimelineItem = new TimelineItem(this);
    }
    ...
    private void createCamera() {
        ...
        for (int i=0;i<supportedResolutions.size();i++){
            if(previewWidth == supportedResolutions.get(i).width){
                parameters.setPreviewSize(supportedResolutions.get(i).width, supportedResolutions.get(i).height);
                Log.i("dx",supportedResolutions.get(i).width + "..." + supportedResolutions.get(i).height);
                break;
            }
        }
    ...
    private void configureMediaStreamFormat() {
        ...
        if (recordAudio) {
            ...
            if (recordVoiceEffect != null){
                capture.addAudioEffect(recordVoiceEffect);
            }
        }
    }
    ...
    public void chooseAudioEffect(View view){
        final String videoLevel[] = new String[]{"无","静音","背景音乐"};
        new AlertDialog.Builder(this)
                .setTitle("请选择")
                .setItems(videoLevel, new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        switch (which){
                            case 0:
                                muteAudioEffect = null;
                                recordVoiceEffect = null;
                                break;
                            case 1:
                                muteAudioEffect = new MuteAudioEffect();
                                muteAudioEffect.setMute(true);
                                recordVoiceEffect = null;
                                break;
                            case 2:
                                muteAudioEffect = null;
                                onOpen(myBackgroundMusicTimelineItem);
                                break;
                        }
                    }
                }).create()
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        if(requestCode == IMPORT_FROM_GALLERY_REQUEST){
            if(myBackgroundMusicTimelineItem.getUri() != null){

                org.m4m.MediaFileInfo mediaFileInfo = new org.m4m.MediaFileInfo(new AndroidMediaObjectFactory(getApplicationContext()));
                try {
                    mediaFileInfo.setUri(myBackgroundMusicTimelineItem.getUri());
                    //Log.i("dx",mediaFileInfo.getAudioFormat())
                } catch (IOException e) {
                    e.printStackTrace();
                }
                org.m4m.AudioFormat audioFormat = new AudioFormatAndroid("audio/mp4a-latm", 44100, 1);
                recordVoiceEffect = new RecordVoiceEffect();
                recordVoiceEffect.setFileUri1(this,myBackgroundMusicTimelineItem.getUri(),audioFormat);
            }
        }
    }
