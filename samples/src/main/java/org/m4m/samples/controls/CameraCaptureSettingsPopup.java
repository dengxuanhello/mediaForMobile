...
public class CameraCaptureSettingsPopup extends Popup {
    ...
    public CameraCaptureSettingsPopup(Context context, List<Camera.Size> resolutions, CameraCaptureSettings listener,int previewWidth) {
        super(context);
        ...
        setupCameraResolutionsSpinner(previewWidth);
    ...
    private void setupCameraResolutionsSpinner(int previewWidth) {
        ...
        for (Camera.Size sz : supportedResolutions) {
            ...
            if(previewWidth == sz.width){
                selection = spinner_data.size() -1 ;
            }
        }
