...
public class RecordVoiceEffect extends AudioEffect {
    ...
    public void applyEffect(ByteBuffer input, long timeProgress) {
        if(reader1.read(byteBuffer1)){
            //audioFormat = new AudioFormatAndroid("audio/mp4a-latm", 48000, 2);
            //if(byteBuffer1.limit()>input.limit()){
                byteBuffer1.limit(input.limit());
            //}
           // Log.i("dx ef", Arrays.toString(byteBuffer1.array()));
            ByteBuffer byteBuffer2 = ByteBuffer.allocate(input.limit());
            byteBuffer2.put(input.array());
            ...
            if (input.capacity() < byteBuffer1.limit()){
                input = ByteBuffer.allocateDirect(byteBuffer1.limit() + 2);
            }

            byteBuffer1.position(0);

            input.position(0);
            input.limit(byteBuffer1.limit());
            input.put(byteBuffer1);*/
        }
    }
